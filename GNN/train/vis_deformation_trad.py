...
def visualize_all_samples(output_dir_name="graphvis"):
    ...
    """
    # --- MODIFIED: Go up one additional directory level to find the 'GNN' root ---
    # This is necessary because the script is now in a subfolder of 'scripts'.
    base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    mesh_dir = os.path.join(base_dir, "data", "meshes_diverse")
    graph_dir = os.path.join(base_dir, "data", "graphs_diverse")
    output_dir = os.path.join(base_dir, output_dir_name) # Create the new directory at the GNN level
